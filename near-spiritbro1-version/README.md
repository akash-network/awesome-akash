...
akash1xw6qvxd4lsmm8kwz8lv0gfsrzemlwnz9zcjly3
...
The image for this `deploy.yaml` file is `image: spiritbro1/near-validator:testnet` testnet, why? because when i first try to deploy it i saw an update in [official validator telegram channel](https://t.me/near_validators/4669) at first i use version 1.23.1 but after the update the testnet is now version 1.24.0 you can check that respectively in their rpc for testnet https://rpc.testnet.near.org/status and for mainnet https://rpc.mainnet.near.org/status and see the `version` value. That's why i differentiate between testnet and mainnet image because the version is different. You can build your own docker image by visiting this repo [here](https://github.com/spiritbro1/near-docker-image), There is two tag for our validator docker image:
