...
for (( ; ; )); do
  ...
  for i in $files; do
    ...
    if grep -q $i /plots/pending.log; then
      ...
      if (("$(ps -aux | grep move | grep -v grep | wc -l)" >= "$TOTAL_UPLOADS")); then
        ...
        START_TIME=$(date +%s)
        curl --connect-timeout 3 --retry 300 --retry-delay 3 -d "filename=$i" -d "endpoint_location=$ENDPOINT_LOCATION" -d "endpoint_directory=$ENDPOINT_DIR" -d "start_time=$START_TIME" -d "provider=$AKASH_CLUSTER_PUBLIC_HOSTNAME" -X POST $JSON_SERVER >>$i.log
      else
  ...
  for i in $pending; do
    ...
    if [[ $JSON_SERVER != "" ]]; then
      curl --connect-timeout 3 --retry 300 --retry-delay 3 -d "filename=$i" -d "progress=$progress" -d "speed=$speed" -X PATCH $JSON_SERVER/$id
    fi
  done
