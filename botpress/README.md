...
- Data is stored in PostgreSQL. Consider regular backups.
- This setup is for a single Botpress instance.
...
- `BP_MODULE_NLU_DUCKLINGURL`: URL for the Duckling server
- `BP_MODULE_NLU_LANGUAGESOURCES`: URL for the Language server
- `CLUSTER_ENABLED`: Set to `false` for single-instance deployment
- `BPFS_STORAGE`: Set to `database` for database storage
